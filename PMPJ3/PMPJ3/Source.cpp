...
void draw_outline() {
	...
	draw_string(GLUT_BITMAP_HELVETICA_12, "Goal : 70(%)", WIDTH * 9.2 / 12, HEIGHT + HEIGHT / 50);
	glLineWidth(5.0f);
	glBegin(GL_LINES);
	glColor3f(1, 1, 1);
...
void renderScene() {
	...
	if (gamemode == 0)
		...
		glColor3f(1, 0.5, 0.1);
		draw_string(GLUT_BITMAP_HELVETICA_12, "Zombie(s) Killed : ", WIDTH / 3.0, HEIGHT + HEIGHT / 50.0 );
		glRasterPos2f(WIDTH / 3.0 + WIDTH / 5, HEIGHT + HEIGHT / 50.0);
		char c[4];
		c[3] = (zombie_killed % 10) + 48;
		for (int i = 0; i < 4; i++)
		...
		draw_outline();
		if (player_life == 0) draw_string(GLUT_BITMAP_HELVETICA_18, "GAME OVER", WIDTH / 3.5, HEIGHT / 5.0 * 3.0);
		else draw_string(GLUT_BITMAP_TIMES_ROMAN_24, "WIN", WIDTH/ 2.6, HEIGHT / 5.0 * 3.0);
		draw_string(GLUT_BITMAP_HELVETICA_18, "RESTART : PRESS ENTER KEY", WIDTH / 4.0, HEIGHT / 6.0 * 1.0);
	}
	glutPostRedisplay();
	glutSwapBuffers();
}
